 ...
 *                 __
 *                 \                    P(x1,x2|y)
 * CMI(X1,X2|Y)= = /_   P(y|x1,x2) log-------------
 *               x1,x2,y              P(x1|y)P(x2|y)
 *
 */
void getCondMutualInf_condition(xxyDist &dist, crosstab<float> &cmi) {
    const double totalCount = dist.xyCounts.count;
    for (CategoricalAttribute x1 = 1; x1 < dist.getNoCatAtts(); x1++) {
        for (CategoricalAttribute x2 = 0; x2 < x1; x2++) {
            float m = 0.0;
            for (CatValue v1 = 0; v1 < dist.getNoValues(x1); v1++) {
                for (CatValue v2 = 0; v2 < dist.getNoValues(x2); v2++) {
                    for (CatValue y = 0; y < dist.getNoClasses(); y++) {
                        const double x1x2y = dist.getCount(x1, v1, x2, v2, y);
                        const double x1x2 = dist.getCount(x1, v1, x2, v2);
                        //                        if (x1x2y && x1x2) {
                        //                            //const unsigned int yCount = dist->xyCounts.getClassCount(y);
                        //                            //const unsigned int  x1y = dist->xyCounts.getCount(x1, v1, y);
                        //                            //const unsigned int  x2y = dist->xyCounts.getCount(x2, v2, y);
                        //                            m += (x1x2y / x1x2) * log2(dist.xyCounts.getClassCount(y) * x1x2y /
                        //                                    (static_cast<double> (dist.xyCounts.getCount(x1, v1, y)) *
                        //                                    dist.xyCounts.getCount(x2, v2, y)));
                        //                        }
                        if (x1x2y) {
                            ...
                            m += ((x1x2y / totalCount) / dist.jointP(x1, v1, x2, v2)) * log2(dist.xyCounts.getClassCount(y) * x1x2y /
                                    (static_cast<double> (dist.xyCounts.getCount(x1, v1, y)) *
                                    dist.xyCounts.getCount(x2, v2, y)));
                        }
                    }
...
double calcF1(crosstab<InstanceCount> &xtab) {

    unsigned int noClasses = xtab[0].size();
    std::vector<double> F1;
    F1.assign(noClasses, 0.0);
    double F1_mean = 0.0;
    ...
    for (unsigned int i = 0; i < noClasses; i++) {
        ...
        for (unsigned int j1 = 0; j1 < noClasses; j1++) {
            for (unsigned int j2 = 0; j2 < noClasses; j2++) {
                TN += xtab[j1][j2];
            }
        }
        TN = TN - TP - FP - FN;
 ...
 * 
 */
void getMutualInformation_avg(xyDist &dist, std::vector<float> &mi) {
    mi.assign(dist.getNoCatAtts(), 0.0);
 ...
 * 
 */
void getCondMutualInf_avg(xxyDist &dist, crosstab<float> &cmi) {
    const double totalCount = dist.xyCounts.count;
    for (CategoricalAttribute x1 = 1; x1 < dist.getNoCatAtts(); x1++) {
        for (CategoricalAttribute x2 = 0; x2 < x1; x2++) {
            float m = 0.0;
            ...
            //            printf("%d\t%d\t%lf\n",x1,x2,m);
            assert(m >= -0.00000001); // CMI is always positive, but allow for some imprecision
            cmi[x1][x2] = m / (dist.getNoValues(x1) * dist.getNoValues(x2));
            cmi[x2][x1] = m / (dist.getNoValues(x1) * dist.getNoValues(x2));
        }
    }
}
 ...
 *               __
 *               \                P(xi,xj)
 *  MI(Xi,Xj)=   /_  P(xi,xj)log------------
 *              xi,xj            P(xi)P(xj)
 *
 *
 */
void getMutualInformation_XiXj(xxyDist &dist, crosstab<float> &mi) {
            ...
            for (CatValue v1 = 0; v1 < dist.getNoValues(x1); v1++) {
                for (CatValue v2 = 0; v2 < dist.getNoValues(x2); v2++) {
                    ...
                    if (x1x2Count) {
                        ...
                        m += (x1x2Count / totalCount) * log2(x1x2Count / ((dist.xyCounts.getCount(x1, v1) / totalCount)
                                * (dist.xyCounts.getCount(x2, v2))));
                    }
                }
            }
...
void mRMR(xxyDist &dist, std::vector<float> &mi, std::vector<CategoricalAttribute> &order_mi, crosstab<float> &mi_xixj, std::vector<CategoricalAttribute> &order_mRMR, FeaSelectionMethod method) {
    std::vector<bool> InOrder;
    InOrder.assign(dist.getNoCatAtts(), false);

    order_mRMR.push_back(order_mi[0]); //mi max attrubute
    InOrder[order_mi[0]] = true;

    int ii = 1;
    while (ii < dist.getNoCatAtts()) {
        //        printf("---------------\n");
        CategoricalAttribute chooseAtt = 0xFFFFFFFFUL;
        double max = -std::numeric_limits<double>::max();
        double valofxixj = 0.0;
        double temp = 0.0;
        for (std::vector<CategoricalAttribute>::const_iterator i = order_mi.begin(); i != order_mi.end(); i++) {
            if (mi[*i] == 0)
                InOrder[*i] = true;
            if (!InOrder[*i]) {
                for (std::vector<CategoricalAttribute>::const_iterator j = order_mRMR.begin(); j != order_mRMR.end(); j++) {
                    valofxixj += mi_xixj[*i][*j];
                }
                valofxixj /= order_mRMR.size();
                switch (method) {
                    case MID:
                        temp = mi[*i] - valofxixj;
                        break;
                    case MIQ:
                        temp = mi[*i] / (valofxixj + 0.0001);
                        break;
                    default:
                        fprintf(stderr,
        ...
        if (chooseAtt != 0xFFFFFFFFUL) {
            //            printf("---%d\n", chooseAtt);
            order_mRMR.push_back(chooseAtt);
            InOrder[chooseAtt] = true;
        }
        ii++;
    }
...
void CalculateClassDistributionloocv(xxxyDist &dist_1, std::vector<std::vector<CategoricalAttribute> > &parents_,
        const CategoricalAttribute x1, std::vector<double> &posteriorDist, const instance &inst) {
    ...
    for (CatValue y = 0; y < noClasses_; y++) {
        if (y != inst.getClass()) {
            if (parents_[x1].size() == 0) {
                ...
                const InstanceCount totalCount1 = dist_1.xxyCounts.xyCounts.getCount(parents_[x1][0], inst.getCatVal(parents_[x1][0]));
                const InstanceCount totalCount1y = dist_1.xxyCounts.xyCounts.getCount(parents_[x1][0], inst.getCatVal(parents_[x1][0]), y); // count[A=v,Y=y]
                if (totalCount1 == 0 || totalCount1y < 2) {
                ...
                const InstanceCount totalCount1 = dist_1.xxyCounts.getCount(parents_[x1][0], inst.getCatVal(parents_[x1][0]), parents_[x1][1], inst.getCatVal(parents_[x1][1]));
                const InstanceCount totalCount1y = dist_1.xxyCounts.getCount(parents_[x1][0], inst.getCatVal(parents_[x1][0]), parents_[x1][1], inst.getCatVal(parents_[x1][1]), y);

                if (totalCount1 == 0 || totalCount1y < 2) {
                    const InstanceCount totalCount2 = dist_1.xxyCounts.xyCounts.getCount(parents_[x1][0], inst.getCatVal(parents_[x1][0]));
                    const InstanceCount totalCount2y = dist_1.xxyCounts.xyCounts.getCount(parents_[x1][0], inst.getCatVal(parents_[x1][0]), y);
                    if (totalCount2 == 0 || totalCount2y < 2) {
                        posteriorDist[y] *= dist_1.xxyCounts.xyCounts.p(x1, inst.getCatVal(x1), y);
                    } else {
            ...
            if (parents_[x1].size() == 0) {
                posteriorDist[y] *= dist_1.xxyCounts.xyCounts.p_loocv(x1, inst.getCatVal(x1), y); // p(a=v|Y=y) using M-estimate

            } else if (parents_[x1].size() == 1) {
                const InstanceCount totalCount1 = dist_1.xxyCounts.xyCounts.getCount(parents_[x1][0], inst.getCatVal(parents_[x1][0]));
                const InstanceCount totalCount1y = dist_1.xxyCounts.xyCounts.getCount(parents_[x1][0], inst.getCatVal(parents_[x1][0]), y); // count[A=v,Y=y]
                if (totalCount1 == 0 || totalCount1y < 2) {
                    posteriorDist[y] *= dist_1.xxyCounts.xyCounts.p_loocv(x1, inst.getCatVal(x1), y);

                } else {
                    posteriorDist[y] *= dist_1.xxyCounts.p_loocv(x1, inst.getCatVal(x1), parents_[x1][0], inst.getCatVal(parents_[x1][0]), y); // p(x1=v1|Y=y, x2=v2) using M-estimate
                }
            } else if (parents_[x1].size() == 2) {
                const InstanceCount totalCount1 = dist_1.xxyCounts.getCount(parents_[x1][0], inst.getCatVal(parents_[x1][0]), parents_[x1][1], inst.getCatVal(parents_[x1][1]));
                const InstanceCount totalCount1y = dist_1.xxyCounts.getCount(parents_[x1][0], inst.getCatVal(parents_[x1][0]), parents_[x1][1], inst.getCatVal(parents_[x1][1]), y);
                ...
                if (totalCount1 == 0 || totalCount1y < 2) {
                    ...
                    if (totalCount2 == 0 || totalCount2y < 2) {
                        ...
                        posteriorDist[y] *= dist_1.xxyCounts.p_loocv(x1, inst.getCatVal(x1), parents_[x1][0], inst.getCatVal(parents_[x1][0]), y);
                    }
                } else {
                    posteriorDist[y] *= dist_1.p_loocv(x1, inst.getCatVal(x1), parents_[x1][0], inst.getCatVal(parents_[x1][0]), parents_[x1][1], inst.getCatVal(parents_[x1][1]), y);
                }
            }
        }
    }
